...
enum AcpStreamEvent {
  ...
  Error { error: String },
  Tool { title: Option<String>, status: Option<String> },
  ...
  for (path, score) in candidates {
    ...
    let validation = validate_paths(
      ...
      settings
        .openclaw_data_dir
        .clone()
        .or_else(|| default_openclaw_data_dir_path().map(|value| value.display().to_string())),
    );
...
fn load_settings(app: AppHandle) -> Result<AppSettings, String> {
  ...
  if settings.openclaw_data_dir.is_none() {
    settings.openclaw_data_dir = default_openclaw_data_dir_path().map(|path| path.display().to_string());
  }

  if settings.openclaw_executable_path.is_none() {
    settings.openclaw_executable_path = detect_first_openclaw_command().map(|path| path.display().to_string());
...
fn save_settings(app: AppHandle, settings: AppSettings) -> Result<AppSettings, String> {
  ...
  normalized_settings.openclaw_executable_path = normalized_settings
    .openclaw_executable_path
    .as_ref()
    .and_then(|value| normalize_openclaw_command_path(Path::new(value)).map(|path| display(&path)));
...
#[tauri::command]
fn read_profile_readme(app: AppHandle, profile_id: String) -> Result<Option<ProfileItemPreview>, String> {
  let settings = load_settings(app.clone())?;
  let root = resolve_profile_root(&app, &settings, &profile_id)?;
...
#[tauri::command]
fn save_profile_readme(app: AppHandle, profile_id: String, content: String) -> Result<ProfileItemPreview, String> {
  let settings = load_settings(app.clone())?;
  let root = resolve_profile_root(&app, &settings, &profile_id)?;
...
fn delete_profile(app: AppHandle, profile_id: String) -> Result<(), String> {
  ...
  settings.recent_launches.retain(|item| item.profile_id != profile_id);
...
#[tauri::command]
fn rename_profile(app: AppHandle, profile_id: String, name: String) -> Result<ManagedProfile, String> {
  if profile_id.is_empty() || profile_id == LOCAL_PROFILE_ID {
...
fn send_chat_message(
  ...
  thread::spawn(move || {
    let result = run_chat_request(&app, &conversation_path, &conversation_id, request, settings);
...
fn emit_streaming_chunks(app: &AppHandle, conversation_id: &str, content: &str) -> Result<(), String> {
...
fn run_agent_streaming_chat(
  ...
  let sdk_path = resolve_acp_sdk_path(&executable_path).ok_or_else(|| "当前 OpenClaw 安装不支持 ACP 真流式输出。".to_string())?;
  ...
  let gateway_config = ensure_target_gateway_running(&runtime_state, &executable_path, &launch_target)?;
  ...
  for line in BufReader::new(stdout).lines() {
    ...
    let event: AcpStreamEvent = serde_json::from_str(&line).map_err(|error| format!("无法解析 ACP 流式消息：{error}"))?;
    match event {
...
fn run_agent_cli_chat(app: &AppHandle, settings: &AppSettings, request: &ChatRequest) -> Result<String, String> {
  let executable = settings
    .openclaw_executable_path
    .clone()
    .ok_or_else(|| "未找到 OpenClaw 启动入口，请先到设置页确认。".to_string())?;
  ...
  let response =
    parsed_response.ok_or_else(|| format!("无法解析龙虾回复.\n{}", raw_output_excerpt(&stdout, &stderr)))?;
...
fn common_openclaw_candidates() -> Vec<(String, i32)> {
  ...
  if cfg!(target_os = "windows") {
    ...
    for (var, score) in [("ProgramFiles", 100), ("LOCALAPPDATA", 90), ("ProgramFiles(x86)", 80)] {
    ...
    candidates.push(("/Applications/OpenClaw.app/Contents/MacOS/openclaw".into(), 100));
    ...
    if let Ok(home) = env::var("HOME") {
      candidates.push((
        PathBuf::from(home).join(".local/bin/openclaw").display().to_string(),
        95,
      ));
    }
  }
...
fn export_profile_impl(request: ExportProfileRequest) -> Result<PackageMeta, String> {
  ...
  let package_name = request
    .package_name
    .filter(|value| !value.trim().is_empty())
    .unwrap_or_else(|| source.file_name().unwrap_or_default().to_string_lossy().to_string());
  let target = request
    .zip_path
    .map(PathBuf::from)
  ...
  let manifest = PackageManifest {
    ...
    exported_at: now_iso(),
    source_dir_name: source.file_name().unwrap_or_default().to_string_lossy().to_string(),
    version: None,
    include_memory,
    include_account_info,
    entries: manifest_entries,
  };
  zip.start_file(MANIFEST_FILE, options).map_err(to_string_error)?;
  zip.write_all(serde_json::to_string_pretty(&manifest).map_err(to_string_error)?.as_bytes())
    .map_err(to_string_error)?;
  ...
  for entry in WalkDir::new(&source) {
    ...
    if entry.file_type().is_dir() {
      zip.add_directory(relative_path, options).map_err(to_string_error)?;
      continue;
    }

    zip.start_file(relative_path, options).map_err(to_string_error)?;
...
fn should_skip_export_path(relative_path: &str, include_memory: bool, include_account_info: bool) -> bool {
...
fn validate_paths_impl(executable_path: Option<String>, data_dir: Option<String>) -> ValidationResult {
...
fn launch_openclaw_impl(
  ...
  if !gateway_ready {
    ...
    thread::spawn(move || {
      ...
      let _ = ensure_target_gateway_running(&runtime_state, &background_executable, &background_target);
    });
  }
...
fn open_control_web_impl(
  ...
  if cfg!(target_os = "windows") {
    ...
    Command::new("open").arg(&url).spawn().map_err(to_string_error)?;
  } else {
...
fn start_gateway_impl(
  ...
  if !gateway_config.url.trim().is_empty() && gateway_port_is_open(&gateway_config) {
    ...
    status.log_tail.push("检测到连接端口已被占用，已跳过重复启动.".into());
    runtime.status = status.clone();
    return Ok(status);
  }
  ...
  status.started_at = Some(now_iso());
  status.log_tail.push(format!("宸插惎鍔ㄨ繛鎺ユ湇鍔″懡浠わ細{command}"));
  status.healthy = health_check_with_retry(&gateway_config, 6, Duration::from_millis(800)).is_ok();
  if !status.healthy {
...
fn gateway_status_impl(_app: AppHandle, state: State<'_, RuntimeState>) -> Result<GatewayStatus, String> {
  let status = {
    let mut runtime = state
      .gateway
      .lock()
...
fn gateway_config_for_target(executable_path: &Path, launch_target: &LaunchTarget) -> Result<GatewayConfig, String> {
  let profile_root = Path::new(&launch_target.profile_path);
  let port = read_gateway_port(profile_root).ok_or_else(|| "没有找到这只龙虾的连接端口配置。".to_string())?;
  let mut args = Vec::new();
  if let Some(profile_name) = launch_target.cli_profile_name.clone() {
    args.push("--profile".into());
    args.push(profile_name);
...
fn resolve_ws_module_path(executable_path: &Path) -> Option<PathBuf> {
  let module_root = resolve_openclaw_module_root(executable_path)?;
  let ws_path = module_root.join("node_modules").join("ws").join("wrapper.mjs");
...
fn proactive_conversation_title(session_key: &str) -> String {
  if session_key.ends_with(":main") {
    ...
    format!("龙虾主动消息 · {}", session_key.rsplit(':').next().unwrap_or(session_key))
  }
}
...
fn append_proactive_message(
  ...
  if conversation
    .messages
    .iter()
    .any(|message| message.role == "assistant" && message.content == content && message.created_at == timestamp)
  {
    return Ok(());
  }
    ...
    for line in BufReader::new(stdout).lines().map_while(Result::ok) {
      ...
      match parsed {
        Ok(GatewaySubscriberEvent::Message {
          ...
          let _ = append_proactive_message(&app_handle, &active_profile_id, &session_key, &text, &timestamp);
        }
...
fn resolve_launch_target(app: &AppHandle, settings: &AppSettings, profile_id: &str) -> Result<LaunchTarget, String> {
  ...
  if profile_id.is_empty() || profile_id == LOCAL_PROFILE_ID {
    let path = settings
      .openclaw_data_dir
      .clone()
      .or_else(|| default_data_dir.as_ref().map(|path| path.display().to_string()))
      .ok_or_else(|| "没有找到默认资料目录，请先确认 C:\\Users\\用户名\\.openclaw 是否存在。".to_string())?;

    if !looks_like_openclaw_data_dir(Path::new(&path)) {
      return Err("默认资料目录无效，请先确认 C:\\Users\\用户名\\.openclaw 是否完整。".into());
    }
...
fn resolve_profile_root(app: &AppHandle, settings: &AppSettings, profile_id: &str) -> Result<PathBuf, String> {
  if profile_id.is_empty() || profile_id == LOCAL_PROFILE_ID {
    let path = settings
      .openclaw_data_dir
      .clone()
...
fn normalize_managed_profile_runtime(profile_root: &Path, profile_id: &str) -> Result<(), String> {
  ...
  if config_path.is_file() {
    ...
    gateway.insert("port".into(), serde_json::Value::Number(gateway_port.into()));
    let auth = gateway
      .entry("auth")
      .or_insert_with(|| serde_json::json!({}))
      .as_object_mut()
...
fn bootstrap_managed_auth(profile_root: &Path) -> Result<(), String> {
  let target = profile_root.join("agents").join("main").join("agent").join("auth-profiles.json");
  if target.is_file() {
    return Ok(());
  }
  ...
  let source = default_root.join("agents").join("main").join("agent").join("auth-profiles.json");
  if !source.is_file() {
    return Ok(());
  }
...
fn is_valid_openclaw_command_path(path: &Path) -> bool {
  ...
  if matches!(file_name.as_str(), "gateway.cmd" | "gateway.bat" | "gateway.exe") {
    return false;
  }
...
fn apply_gateway_defaults(settings: &mut AppSettings) {
  ...
  let gateway_command = data_dir.join("gateway.cmd");
  if settings
    .gateway_config
    .command
    .as_ref()
  ...
  if settings.gateway_config.command.is_none() && gateway_command.is_file() {
    settings.gateway_config.command = Some(gateway_command.display().to_string());
  }
...
fn health_check_with_retry(config: &GatewayConfig, attempts: usize, delay: Duration) -> Result<(), String> {
...
fn migrate_legacy_profiles_if_needed(app: &AppHandle, settings: &AppSettings) -> Result<(), String> {
...
fn schedule_summary_from_value(value: &serde_json::Value) -> Option<String> {
  match value {
    serde_json::Value::String(_) | serde_json::Value::Bool(_) | serde_json::Value::Number(_) => {
      json_value_to_compact_text(value)
    }
...
fn read_package_manifest<R: Read + std::io::Seek>(
  ...
  manifest_file.read_to_string(&mut contents).map_err(to_string_error)?;
...
fn collect_notifications(root: &Path) -> Result<Vec<NotificationItem>, String> {
  ...
  items.sort_by(|left, right| right.created_at.cmp(&left.created_at).then_with(|| right.id.cmp(&left.id)));
  items.truncate(100);
  Ok(items)
}

fn collect_cron_run_notifications(root: &Path, output: &mut Vec<NotificationItem>) -> Result<(), String> {
  ...
  for entry in fs::read_dir(runs_root).map_err(to_string_error)? {
    ...
    for line in BufReader::new(File::open(&path).map_err(to_string_error)?).lines().map_while(Result::ok) {
      if line.trim().is_empty() {
        continue;
      }
      ...
      let job_id = value.get("jobId").and_then(|value| value.as_str()).unwrap_or("cron");
...
fn collect_heartbeat_notifications(root: &Path, output: &mut Vec<NotificationItem>) -> Result<(), String> {
  let sessions_path = root.join("agents").join("main").join("sessions").join("sessions.json");
  ...
  for (session_key, entry) in entries {
    ...
    let Some(mut content) = latest_assistant_text_from_session_file(Path::new(session_file))? else {
      continue;
    };
...
fn preview_cron_item(root: &Path, item_id: &str) -> Result<ProfileItemPreview, String> {
  ...
  Ok(ProfileItemPreview {
    title,
    subtitle,
    content: serde_json::to_string_pretty(&localize_preview_json_times(job)).map_err(to_string_error)?,
...
fn preview_memory_item(root: &Path, item_id: &str) -> Result<ProfileItemPreview, String> {
  ...
  let file_name = safe_item_name(item_id)?;
  let path = root.join("agents").join("main").join("sessions").join(file_name);
  if !path.is_file() {
...
fn preview_account_item(root: &Path, item_id: &str) -> Result<ProfileItemPreview, String> {
  if let Some(key) = item_id.strip_prefix("auth-") {
    let path = root.join("agents").join("main").join("agent").join("auth-profiles.json");
    let value = read_json::<serde_json::Value>(&path)?;
    let profile = value
      .get("profiles")
    ...
    let provider = profile
      ...
      .and_then(|item| item.as_str())
      .unwrap_or("未知提供方");
    let auth_type = profile.get("type").and_then(|item| item.as_str()).unwrap_or("未知类型");
    return Ok(ProfileItemPreview {
      title: key.to_string(),
  ...
  if let Some(device_id) = item_id.strip_prefix("device-") {
    ...
    let client = device.get("clientId").and_then(|item| item.as_str()).unwrap_or("unknown");
    let platform = device.get("platform").and_then(|item| item.as_str()).unwrap_or("unknown");
    return Ok(ProfileItemPreview {
...
fn collect_skill_items(root: &Path) -> Result<Vec<ProfileListItem>, String> {
  ...
  let mut items = Vec::new();
  for entry in fs::read_dir(&workspace).map_err(to_string_error)? {
    let entry = entry.map_err(to_string_error)?;
    let path = entry.path();
    if !path.is_file() {
      continue;
...
fn collect_memory_items(root: &Path) -> Result<Vec<ProfileListItem>, String> {
  ...
  items.sort_by(|left, right| right.updated_at.cmp(&left.updated_at).then_with(|| left.title.cmp(&right.title)));
  Ok(items)
}
...
fn collect_account_items(root: &Path) -> Result<Vec<ProfileListItem>, String> {
  ...
  let auth_profiles = root.join("agents").join("main").join("agent").join("auth-profiles.json");
...
mod tests {
  use super::{
    cli_profile_name_for, collect_cron_items, export_profile_impl, is_valid_cli_profile_name,
    extract_agent_cli_text, localize_preview_json_times, normalize_managed_profile_runtime,
    preview_cron_item, read_json, schedule_summary_from_value, should_skip_export_path,
    verify_import_package_impl, ExportProfileRequest,
  };
  ...
  fn export_defaults_skip_memory_and_account_paths() {
    assert!(should_skip_export_path("agents/main/sessions/item.jsonl", false, false));
    assert!(should_skip_export_path("devices/paired.json", false, false));
    assert!(should_skip_export_path("identity/device-auth.json", false, false));
    assert!(should_skip_export_path("agents/main/agent/auth-profiles.json", false, false));
    assert!(!should_skip_export_path("workspace/project.txt", false, false));
  }

  #[test]
  fn export_can_include_sensitive_paths_when_enabled() {
    assert!(!should_skip_export_path("agents/main/sessions/item.jsonl", true, false));
    assert!(!should_skip_export_path("devices/paired.json", false, true));
    assert!(!should_skip_export_path("identity/device-auth.json", false, true));
    assert!(!should_skip_export_path("agents/main/agent/auth-profiles.json", false, true));
  }
  ...
  fn export_profile_excludes_sensitive_files_by_default() {
    ...
    fs::write(source.join("agents/main/agent/auth-profiles.json"), "secret").unwrap();
    ...
    assert!(!names.iter().any(|name| name.contains("sessions/history.jsonl")));
    assert!(!names.iter().any(|name| name == "USER.md"));
    assert!(!names.iter().any(|name| name.contains("auth-profiles.json")));
    assert!(!names.iter().any(|name| name.contains("devices/paired.json")));
    assert!(!names.iter().any(|name| name.contains("identity/device-auth.json")));
  ...
  fn read_json_accepts_utf8_bom() {
    ...
    assert_eq!(value.get("value").and_then(|item| item.as_bool()), Some(true));
  ...
  fn verify_import_package_detects_tampering() {
    ...
    for entry in WalkDir::new(&unpacked) {
      ...
      let relative = path.strip_prefix(&unpacked).unwrap().to_string_lossy().replace('\\', "/");
      if entry.file_type().is_dir() {
  ...
  fn normalize_runtime_removes_invalid_gateway_remote_mode() {
    let root = env::temp_dir().join(format!("openclaw-gateway-normalize-test-{}", Uuid::new_v4()));
    fs::create_dir_all(&root).unwrap();
    fs::write(
